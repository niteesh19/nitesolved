...
//    moreFunnyExamples();
  ...
  static class Item { public int x; public Item(int x) { this.x = x; } }
  public static void immutableEx() {
    var list = List.of(new Item(1), new Item(2));
    System.out.println(list);
    var newList = list.stream().peek(item -> item.x = 3).collect(toList());
    System.out.println(list);
  ...
  private static void mapExamples() {
    ...
    List<Integer> ints = Arrays.stream(s.split("\\s+")).map(Integer::parseInt).collect(toList());
    System.out.println(ints);

    System.out.println("2. Mapping with a simple lambda expression:");
    List<Double> pow2 = ints.stream().map(x -> Math.pow(2.0, x)).collect(toList());
    System.out.println(pow2);
    List<String> doubled = ints.stream().map(i -> i + "_" + i).collect(toList());
  ...
  private static void filterExamples() {
    ...
    List<String> longWords = words.stream().filter(s -> s.length() > 3).collect(toList());
  ...
  private static void moreFunnyExamples() {
    ...
    List<String> sorted = Stream.of("a", "z", "b", "c").sorted().collect(toList());
    System.out.println(sorted);

    System.out.println("6. Limit and skip");
    List<Integer> part = Stream.of(1, 2, 3, 4, 5, 7, 8, 9).skip(2).limit(4).collect(toList());
    ...
    List<Integer> range = IntStream.range(1, 10).mapToObj(i -> i).collect(toList());
